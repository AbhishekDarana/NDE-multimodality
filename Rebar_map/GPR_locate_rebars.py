"""Abhishek Darana-Added save procedures to the CHARISMA code to plot data for rebars"""
...
def read_csv(directory):
    ...
    filepath_data = directory
    val=directory.split(".csv")[0]
    filepath_config = os.path.join(os.path.dirname(directory),"config"+val[-1]+".csv")
...
def locate_rebar_consecutive(save_path,filename_prefix,migrated_data, velocity, rhf_range, rh_nsamp, profilePos, 
    ...
    plt.savefig(os.path.join(save_path,filename_prefix+"_unmarked.jpg"))
    scatter = ax.scatter(filtered_cluster_centers[:, 0], filtered_cluster_centers[:, 1],
                         c='red', marker='o', s=30, edgecolors='black')
    plt.savefig(os.path.join(save_path,filename_prefix+".jpg"))
    config = configparser.ConfigParser()
    config['CENTERS'] ={}
    config['CENTERS']['Points'] = str(cluster_centers_inches_m)
    config['CENTERS']['datatype'] = str(type(cluster_centers_inches_m))
    with open(os.path.join(save_path,(filename_prefix+".ini")), 'w') as configfile:
